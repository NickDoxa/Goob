...
One round trip per call. The arm is exposed as the `move_arm` tool; Claude
chooses whether to call it. We collect text blocks into `text` and the first
`move_arm` tool_use block into `movement`. Multiple tool calls in a single
response are ignored past the first.
"""
...
def ask_claude(user_text: str, image_jpeg: bytes) -> ClaudeResponse:
    ...
    return ClaudeResponse(text="\n".join(text_parts).strip(), movement=movement)
